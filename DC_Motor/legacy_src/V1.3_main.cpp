...
{
  analogWrite(EN1, e1_2); // モータの回転数を制御 speed control
  analogWrite(M1A, m1_2);
  analogWrite(M2A, m2_2);
  delay(dly_2);
}
...
void loop()
{
  for (int i = 0; i < 5; i++)
  {
    ...
    Serial.println("正転0.5秒");
    Motor1(HIGH, HIGH, LOW, 500); // 正転one way
    Serial.println("逆転0.5秒");
    Motor1(HIGH, LOW, HIGH, 500); // 逆転reverse
  }
  Serial.println("緩停止2秒");
  Motor1(LOW, LOW, LOW, 2000); // 緩停止 緩停止slow stop

  Serial.println("正転3秒");
  Motor1(HIGH, HIGH, LOW, 3000); // 正転one way

  Serial.println("緩停止1秒");
  Motor1(LOW, HIGH, LOW, 1000);  // 緩停止slow stop

  Serial.println("逆転3秒");
  Motor1(HIGH, LOW, HIGH, 3000);   // 逆転reverse

  Serial.println("急停止2秒");
  Motor1(HIGH, LOW, LOW, 2000);   // 急停止fast stop

  Serial.println("0.1秒毎に徐々に増速");
  for (int j = 0; j < 256; j++)
  {
    //---PWM example, full speed then slow
    Motor2(j, HIGH, LOW, 100); // EN1 ON , 正転one way
  }

  Serial.println("0.1秒毎に徐々に減速");
  for (int k = 255; k < 0; k--)
  {
    Motor2(k, HIGH, LOW, 100); // EN1 ON , 正転one way
  }
